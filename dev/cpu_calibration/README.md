...
construction 的独立批次已经完成：12 次正式耗时 6.493–8.471 秒，独立审计通过，
自有资源已清理。时间尺度已补齐，固定计算仍有变化；不据此自动重跑 HNSW。
见[全部结果与解释边界](results/20260913T030031Z-ce171210b519/report.md)。
...
当前 7 项 profile/解析器测试（含只改协议文件的变异验证）和 3 项清理测试通过。
协议起止摘要检查在实际测量版本中已经存在，补充验证没有修改执行源码。
见[开发与初始测试](review-construction-profile/manifest.json)及
[协议保护补充验证](review-protocol-freeze/report.md)。

独立标准库审计不导入运行入口，不启动任何容器或计算；10 项变异自测通过，
见[审计开发证据](review-construction-audit/manifest.json)。复核已有结果可执行：

```bash
.venv-glove/bin/python dev/cpu_calibration/audit.py \
  dev/cpu_calibration/results/20260913T030031Z-ce171210b519
```

仅当目标目录尚无审计输出时才使用 `--write`；已有 `independent-review.json` 和
`report.md` 不会被覆盖。
