...
/* Keep validation independent of the calculation so boundaries can be tested
 * without launching participants or executing any distance iterations. */
static int parse_iterations(const char *text, uint64_t *iterations)
{
    ...
    if (errno || end == text || *end || value < 1 || value > UINT64_C(4000000000)) return 0;
    *iterations = value;
    return 1;
}
...
int main(int argc, char **argv)
{
    ...
    if (!parse_iterations(argv[1], &iterations)) return 2;
